...
final firebaseEncryptionServiceProvider = Provider<FirebaseEncryptionService>((ref) {
  final firebaseKey = ref.watch(firebaseKeyProvider);
  return FirebaseEncryptionService(firebaseKey ?? 'default_firebase_sec_key_123456');
});
...
class FirebaseEncryptionService {
  ...
  FirebaseEncryptionService(this._rawKey) {
    ...
    _keyBytes = digest.bytes.sublist(0, 16); // Take the first 16 bytes for AES-128
  }
